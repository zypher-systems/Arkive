...
func TestDispatch(t *testing.T) {
	...
	for _, name := range []string{"export", "gc"} {
		if code := dispatch([]string{name}); code != 2 {
			t.Fatalf("%s stub exit=%d", name, code)
		}
	}
	if code := dispatch([]string{"user", "reset-2fa", "a@b.c"}); code != 2 {
		t.Fatalf("reset-2fa stub exit=%d", code)
	}
